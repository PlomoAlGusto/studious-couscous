...
# -----------------------------------------------------------------------------
st.set_page_config(page_title="Quimera Pro v17.2 Final", layout="wide", page_icon="🧠")
st.markdown("""
<style>
...
# =============================================================================
# DERIVADOS: PROMEDIO PONDERADO DE 4 FUENTES
# =============================================================================
...
# =============================================================================
# BACKTESTING CORREGIDO (SIN ERRORES)
# =============================================================================
def run_backtest(df, filters, atr_threshold=None):
    ...
    for i in range(50, len(df_bt)):
        ...
        # === CORRECCIÓN CLAVE: asegurar que ADX_14 es un escalar ===
        adx_val = row['ADX_14']
        if isinstance(adx_val, pd.Series):
            adx_val = adx_val.iloc[0] if len(adx_val) > 0 else 0
...
# =============================================================================
# NUEVO: CRYPTOPANIC + CALENDARIO ECONÓMICO
# =============================================================================
@st.cache_data(ttl=600)
def get_cryptopanic_news(symbol, max_items=5):
    api_key = st.secrets.get("CRYPTOPANIC", {}).get("API_KEY")
    if not api_key:
        return [{"title": "⚠️ API Key no configurada", "source": "CryptoPanic", "sentiment": 0, "url": "#"}]
    base = symbol.split('/')[0].upper()
    url = f"https://cryptopanic.com/api/v1/posts/?currencies={base}&kind=news&limit={max_items}&auth_token={api_key}"
    try:
        resp = requests.get(url, timeout=8)
        if resp.status_code != 200: return [{"title": "⚠️ Error API", "source": "CryptoPanic", "sentiment": 0, "url": "#"}]
        data = resp.json()
        results = []
        ...
        logging.error(f"CryptoPanic error: {e}")
        return [{"title": "⚠️ Error de conexión", "source": "CryptoPanic", "sentiment": 0, "url": "#"}]

@st.cache_data(ttl=3600)
def get_economic_calendar():
    try:
        resp = requests.get("https://nfs.faireconomy.media/ff_calendar_thisweek.json", timeout=10)
        events = [e for e in resp.json() if e.get('impact') == 'High' and 'forecast' in e]
        return events[:5]
    except:
        return [{"title": "No hay eventos esta semana", "country": "🌐", "forecast": "", "date": ""}]

# -----------------------------------------------------------------------------
# 3. INTERFAZ SIDEBAR
# -----------------------------------------------------------------------------
with st.sidebar:
    st.title("🧠 QUIMERA v17.2")
...
# -----------------------------------------------------------------------------
# 5. IA ANALISTA CON ALERTAS DE FUNDING EXTREMO Y CVD
# -----------------------------------------------------------------------------
def generate_detailed_ai_analysis_html(row, mtf_trends, mtf_score, cvd, fr, open_interest, data_src, regime, bayes_prob, is_liquid):
    ...
    html = f"""
    <div class='ai-box'>
        <span class='ai-title'>🤖 QUIMERA COPILOT v17.2 (ML + CVD + FR Alert):</span>
...
def run_strategy(df, cvd, trend_4h, filters, fr, is_liquid, atr_vol_threshold):
    ...
    # === FILTRO ESTRITO DE TENDENCIA ===
    if trend_4h == "BULLISH" and signal == "SHORT": signal = "NEUTRO"
    if trend_4h == "BEARISH" and signal == "LONG": signal = "NEUTRO"
...
if df is not None:
    ...
    # === ESTRATEGIA OPTIMIZADA CON R:R ASIMÉTRICO ===
    ...
    # === NOTICIAS: CRYPTOPANIC + CALENDARIO ===
    ...
    if calc_dir:
        # === R:R ASIMÉTRICO ===
        sl_dist = atr * 1.0    # SL más ajustado
        tp1_dist = atr * 1.5  # TP más amplio
        tp2_dist = atr * 3.0
        tp3_dist = atr * 5.0
        risk = sl_dist
...
"""
    ...
    tab1, tab2, tab3 = st.tabs(["📊 LIVE COMMAND", "📰 NOTICIAS + MACRO", "🧪 PAPER TRADING"])
    with tab1:
        col_tech, col_fng = st.columns([1, 1])
        with col_tech:
...
"""
    ...
    with tab2:
        st.markdown("### 📰 CryptoPanic (Sentimiento)")
        for n in cp_news:
            color = "#00FF00" if n['sentiment'] > 0.3 else "#FF4444" if n['sentiment'] < -0.3 else "#FFFF00"
            icon = "🟢" if n['sentiment'] > 0.3 else "🔴" if n['sentiment'] < -0.3 else "🟡"
            st.markdown(f"{icon} <a href='{n['url']}' target='_blank' style='color:{color}'>{n['title']}</a>", unsafe_allow_html=True)
        st.divider()
        st.markdown("### 📅 Calendario Económico")
        for ev in econ_events:
            st.markdown(f"**{ev['title']}** ({ev.get('country', '')})")
            if ev.get('forecast'): st.caption(f"Previsto: {ev['forecast']}")
...
if auto_refresh: time.sleep(60); st.rerun()# GESTIÓN DE ARCHIVOS
CSV_FILE = 'paper_trades.csv'
COLUMNS_DB = ["id", "time", "symbol", "type", "entry", "size", "leverage", "sl", "tp1", "tp2", "tp3", "status", "pnl", "reason", "candles_held", "atr_entry"]
INITIAL_CAPITAL = 10000.0
