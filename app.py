...
<style>
    ...
    .trade-card-box {
        ...
        border-radius: 10px !important; padding: 25px !important; margin-bottom: 20px !important;
...
def display_market_sessions():
    ...
    for name, (start, end) in sessions.items():
        ...
        st.sidebar.markdown(f"<div style='font-size:11px; padding:5px; margin-bottom:5px; background:{'rgba(50,255,50,0.1)' if is_open else 'rgba(255,255,255,0.05)'}; border-radius:4px; display:flex; justify-content:space-between;'><span>{name}</span><span>{'🟢' if is_open else '🔴'}</span></div>", unsafe_allow_html=True)
...
def render_quimera_ai(regime, atr, fr, fng, rsi, trend_strength, adr_val, tsi_val, mfi_val):
    c_reg = "#3fb950" if "TENDENCIA" in regime else "#d29922"
    
    # Lógica visual para nuevos indicadores
    tsi_col = "#3fb950" if tsi_val > 0 else "#f85149"
    tsi_txt = "Bullish" if tsi_val > 0 else "Bearish"
    
    mfi_col = "#3fb950" if mfi_val < 80 and mfi_val > 20 else "#d29922"
    
    trend_change_icon = "🔄" if abs(tsi_val) < 10 else "➡️" 
    
    html = f"""
    <div style="margin-bottom:10px; font-weight:bold; color:#a371f7; display:flex; align-items:center; gap:5px; font-size:14px;">
        <span>🧠 QUIMERA AI ANALYSIS (EXTENDED)</span>
    </div>
    <div style="background-color:#161b22; border-top:3px solid #a371f7; padding:15px; border-radius:0 0 6px 6px; margin-bottom:20px;">
        ...
        <div style="display:flex; justify-content:space-between; padding:6px 0; border-bottom:1px solid #30363d;">
            <span style="color:#8b949e">📊 Fuerza Tendencia</span><span style="color:#e6edf3; font-weight:bold">{trend_strength}</span>
        </div>
        <div style="display:flex; justify-content:space-between; padding:6px 0; border-bottom:1px solid #30363d;">
            <span style="color:#8b949e">📏 ADR (Rango Diario)</span><span style="color:#e6edf3; font-weight:bold">{adr_val:.2f}%</span>
        </div>
        <div style="display:flex; justify-content:space-between; padding:6px 0; border-bottom:1px solid #30363d;">
            <span style="color:#8b949e">🚀 TSI (Momento)</span><span style="color:{tsi_col}; font-weight:bold">{tsi_val:.2f} ({tsi_txt})</span>
        </div>
        <div style="display:flex; justify-content:space-between; padding:6px 0; border-bottom:1px solid #30363d;">
            <span style="color:#8b949e">💸 MFI (Volumen)</span><span style="color:{mfi_col}; font-weight:bold">{mfi_val:.1f}</span>
        </div>
         <div style="display:flex; justify-content:space-between; padding:6px 0; border-bottom:1px solid #30363d;">
            <span style="color:#8b949e">🔮 Cambio Tendencia</span><span style="color:#e6edf3; font-weight:bold">{trend_change_icon}</span>
        </div>
        <div style="display:flex; justify-content:space-between; padding:6px 0; border-bottom:1px solid #30363d;">
            <span style="color:#8b949e">🌡️ Sentimiento</span><span style="color:#e6edf3; font-weight:bold">{fng}</span>
        </div>
        <div style="display:flex; justify-content:space-between; padding:6px 0;">
            <span style="color:#8b949e">💢 Volatilidad (ATR)</span><span style="color:#e6edf3; font-weight:bold">${atr:.2f}</span>
        </div>
    </div>
    """
    return textwrap.dedent(html)
...
def main():
    ...
    signal_raw, atr, details, regime = strat_mgr.get_signal(df, {})
    ...
    with col2:
        ...
        # Datos extra que calculamos en strategy.py
        ...
        st.markdown(render_quimera_ai(regime, atr, fr, fng_val, rsi_val, trend_str, adr_val, tsi_val, mfi_val), unsafe_allow_html=True)
