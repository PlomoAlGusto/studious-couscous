...
# =============================================================================
# NOTICIAS: COMBINADAS (Cointelegraph + CryptoPanic)
# =============================================================================
...
def get_combined_news(symbol):
    ...
    try:
        ...
        for e in feed.entries[:10]:  # Tomamos 10 de CoinTelegraph
            coin_telegraph.append({
                "title": e.title,
                "link": e.link,
                "published": e.published_parsed,
                "source": "Cointelegraph",
                "sentiment": sia.polarity_scores(e.title)['compound']
            })
    except Exception as e:
        logging.error(f"CoinTelegraph error: {e}")
    ...
    if api_key:
        ...
        url = f"https://cryptopanic.com/api/v1/posts/?currencies={base}&kind=news&limit=5&auth_token={api_key}"
        try:
    ...
    all_news = coin_telegraph + crypto_panic
    seen_titles = set()
    unique_news = []
    ...
    # Ordenar por fecha (si es posible) y limitar a 10
    unique_news.sort(key=lambda x: x.get('published', time.gmtime()), reverse=True)
...
with st.sidebar:
    ...
    get_market_sessions()
    st.divider()
    symbol = st.text_input("Ticker", "BTC/USDT", key="ticker_input")  # ⚠️ KEY AÑADIDA
...
if df is not None:
    ...
    # --- NOTICIAS COMBINADAS (CoinTelegraph + CryptoPanic) ---
    combined_news = get_combined_news(symbol)
...
"""
    ...
    with tab1:
        ...
        with col_news:
            st.markdown("### 📰 NOTICIAS (CoinTelegraph + CryptoPanic)")
            if combined_news:
                for n in combined_news:
                    ...
                    st.markdown(f"{icon} <a href='{n['link']}' target='_blank' style='color:{color}; text-decoration: none;'>{n['title']}</a> <span style='font-size:10px; color:#888'>({n['source']})</span>", unsafe_allow_html=True)
            else:
