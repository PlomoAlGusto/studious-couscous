...
with st.sidebar:
    ...
    st.markdown("---")
    tg_status = config.TELEGRAM_TOKEN and config.TELEGRAM_CHAT_ID
    if tg_status:
        st.markdown(
            """<div style='text-align:center; color:#00FF00; font-size:11px; 
            border:1px solid #005500; background:rgba(0,255,0,0.1); 
        ...
        st.markdown(
            """<div style='text-align:center; color:#FF4444; font-size:11px; 
            border:1px solid #550000; background:rgba(255,0,0,0.1); 
            padding:6px; border-radius:4px; font-weight:bold;'>
            🔕 TELEGRAM: OFFLINE</div>""", 
            unsafe_allow_html=True
        )
...
def main():
    ...
    with col1:
        ...
        if c_btn2.button(f"🚀 EJECUTAR {display_signal}", use_container_width=True, type=btn_type):
            ...
            # Notificación automática a Telegram si es Diamante
            if signal_strength == "DIAMOND":
                with st.spinner("Notificando..."): send_telegram_alert(symbol, display_signal, price, sl, tp1, opt_lev)
            else:
                # Si es Potencial, lo mandamos igual porque el usuario pulsó el botón
                with st.spinner("Notificando..."): send_telegram_alert(symbol, f"{display_signal} (MANUAL)", price, sl, tp1, opt_lev)
